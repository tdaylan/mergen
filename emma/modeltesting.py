...
n_bins = 40 # >> histogram
kernel_vis = True
output_dir = "./plots/plots032420-3/"
prefix = "sector20-"
...
for j in range(len(noise)):
    ...
    if fake_data: # -- artificial data -----------------------------------------
        ...
        if classified:
            ...
            plt.ylabel('relative flux')
            plt.legend()
            plt.savefig(output_dir + prefix + 'train_data.png')
            plt.close()
            
            plt.figure()
            inds1 = np.nonzero(y_test == 1.)[0]
            inds2 = np.nonzero(y_test == 2.)[0]
            inds3 = np.nonzero(y_test == 3.)[0]
            inds4 = np.nonzero(y_test == 4.)[0]
            for i in inds1:
                plt.plot(x, x_test[i], 'r-', alpha = 0.5, label = '1')
            for i in inds2:
                plt.plot(x, x_test[i], 'b-', alpha = 0.5, label = '2')
            for i in inds3:
                plt.plot(x, x_test[i], 'g-', alpha = 0.5, label = '3')
            for i in inds4:
                plt.plot(x, x_test[i], 'm-', alpha = 0.5, label = '4')
            plt.xlabel('time [days]')
            plt.ylabel('relative flux')
            plt.legend()
